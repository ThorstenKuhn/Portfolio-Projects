...
def write_to_box(char_):
    ...
    #check if char_ is valid input
    if (char_ == "." and ("." in full_text_box or full_text_box == "")) or char_ not in "^0123456789.()/*-+": #check if char_ is valid input
        return
    
    # check if input(char_) is to append into box(calc_input)
    elif char_ in "0123456789.": 
        calc_input.insert('end-1c', char_)
        
    # check if nothing to append to equation, change last operator in equation
    elif calc_output.cget('text')[-1:] in "/*-+^" and full_text_box == "" and equation != "":
        calc_output.config(text=calc_output.cget('text')[0:-1] + char_)
        
    # check if the input has a number and can added with the operator into equation
    elif full_text_box != "" or equation != "":
        add_to_output(char_)
...
def evaluating():
    ...
    equation = calc_output.cget('text')
    while equation.count('(') > equation.count(')'): # closing all open brackets
        equation += ')'
    try:
        # evaluate the equation with translated power operator
        result = numexpr.evaluate(equation.replace('^', '**')) 
...
def bracket_l():
    ...
    # check if last character is digit and missed a operator and add '*'
    if input_string[-1:] in "0123456789" and input_string != "":
        add_to_output("*(")
    else:
        add_to_output("(")
    clear()
...
calc_output = ttk.Label(root, text = "", width=12, anchor="e") # label for equation
